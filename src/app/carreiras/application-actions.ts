...
export async function submitCareerJobApplicationAction(formData: FormData) {
  ...
  const parsed = careerApplicationLogisticsSchema.safeParse({
    ...
    availabilityShifts: formData.getAll("availability_shifts").map(String),
    availableFrom: field(formData, "available_from"),
    isReferred: field(formData, "referral_status") === "yes",
    referredBy: field(formData, "referred_by"),
  ...
  const { data: applicationId, error } = await supabase.rpc(
    "submit_career_job_application_with_logistics",
    {
      ...
      p_availability_shifts: parsed.data.availabilityShifts,
      p_available_from: parsed.data.availableFrom,
      p_is_referred: parsed.data.isReferred,
      p_referred_by: parsed.data.isReferred ? parsed.data.referredBy : null,
