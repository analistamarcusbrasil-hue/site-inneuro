...
-- Uma candidatura continua sendo um único registro durante toda a vaga.
...
-- As filas operacionais usam somente a etapa atual. Candidaturas retiradas
-- continuam disponíveis em "Todos", mas não são trabalho pendente de uma etapa.
  ...
  with candidates as (
    ...
    where application.job_id = p_job_id
      ...
      and (
        p_stage is null
        or (
          application.candidate_stage = p_stage
...
begin
  ...
  into unique_ids
  from unnest(coalesce(p_application_ids, '{}'::uuid[])) id;
  ...
  if p_expected_stage not in ('resume', 'interview', 'practical_test', 'hiring') then
    raise exception using errcode = '22023', message = 'invalid_expected_stage';
  end if;
  if exists (
    select 1
    from unnest(unique_ids) selected_id
    left join public.career_job_applications application
      on application.id = selected_id
    where application.id is null
      or application.job_id <> p_job_id
      or application.candidate_stage <> p_expected_stage
  ) then
