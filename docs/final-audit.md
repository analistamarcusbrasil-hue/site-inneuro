...
## Complemento editorial — 23 de julho de 2026

- A auditoria e as páginas legais da `main` foram integradas ao painel editorial.
- Foram criadas `/noticias` e `/noticias/[slug]`, com capa otimizada, metadados,
  canonical, `noindex` automático para a listagem vazia e 404 para slug não
  publicado.
- Notícias em destaque e chamadas de redes sociais passaram a exibir as mídias
  selecionadas no painel. Rascunhos não são consultados com a sessão do usuário
  administrativo.
- Slides e convênios estáticos já aprovados permanecem como fallback enquanto o
  respectivo módulo do CMS não tiver nenhum registro publicado.
- A rota `/api/health/cms` foi validada contra o Supabase e respondeu
  `{"configured":true,"database":"connected"}`.
- A biblioteca de mídia mantém validação de assinatura, MIME, extensão e tamanho;
  falhas de atualização, exclusão, restauração e publicação agora são tratadas no
  fluxo administrativo.
- O build final gerou 41 páginas. Home, convênios, notícias, 404, páginas legais e
  login administrativo foram conferidos com um `main`, um `h1`, sem conteúdo
  provisório e sem overflow horizontal.
- A Home foi conferida em 360, 390, 768, 1024, 1440 e 1920 px. O menu móvel foi
  validado com foco inicial, Escape, restauração do foco e bloqueio da rolagem.
- `npm run lint`, `npm run typecheck`, `npm run format:check` e `npm run build`:
  aprovados.

Pendências operacionais antes de a equipe publicar pelo painel:

- [ ] Aplicar a migration `202607230001_cms_health.sql` no projeto Supabase para
      manter o esquema remoto alinhado ao repositório.
- [ ] Configurar `SUPABASE_SERVICE_ROLE_KEY` como segredo de servidor na Vercel
      para habilitar convites e alteração de funções.
- [ ] Confirmar ou criar o primeiro usuário `super_admin` conforme
      `docs/admin-cms.md`.
- [ ] Fazer um teste autenticado de upload, rascunho, revisão e publicação com a
      conta oficial da clínica.
